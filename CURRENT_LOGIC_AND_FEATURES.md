...
> 기준 버전: MY9 `1.2.0+10` / API `1.2.0` (`v1.1` 기준선 이후)
> 작성 기준일: 2026-07-26
...
- 다음 경기 영역을 누르면 해당 날짜의 경기 일정으로 이동
- 응원팀에 따라 4개 메인 섹션 아이콘 변경
- 두산 섹션 아이콘은 런타임 토글 없이 빌드별로 고정
  - `DOOSAN_SECTION_THEME=cheolwoong`: 철웅이 섹션 아이콘
  - `DOOSAN_SECTION_THEME=mangom`: 망곰 섹션 아이콘
...
- 전체 직관 승·무·패와 승률
- 요일별·구장별 승률
- 상대 구단별 직관 승률과 승·무·패 요약
- 직관 하단 경기 리스트를 전체/승/무/패 탭으로 분할 조회
- 직관 경기 범위에서 타자 TOP 5:
  - 홈런
  - 타점
  - 안타
...
- 직관 경기 범위에서 투수 TOP 5:
  ...
  - 승수
  - 투구 이닝
- 직관 섹션은 짧은 표본에서도 바로 해석 가능한 누계형 기록을 우선 사용한다.
  OPS, 출루율, 장타율, K/9, 피안타율처럼 표본이 길수록 의미가 강해지는 비율 지표는
  시즌 기록/상세 분석에서 확인한다.
...
| 10초마다 | 실시간 점수/이닝/진행 상태 | 경기 30분 전부터 종료 예상 6시간까지 |
| 매일 00:00 | 전일 완료 경기 박스스코어 | 어제 |
| 컨테이너 시작 시 | 누락 복구 backfill | 3월 1일~오늘(최소 최근 30일) |
| 컨테이너 시작 시/매일 06:10 | 미래 일정 보강 | 오늘~11월 30일 |
...
- 규정 타석: 팀 경기 수 × 3.1을 가장 가까운 정수 타석으로 반올림
...
- `K-BB% = (K - BB) / BF × 100`
- 전체 선수 목록은 기존 방식으로 표시하고 TOP 5만 규정타석/규정이닝 체크박스로 적용 여부를 전환
...
| 직관 | `GET /v1/attendances/summary` | 승요·TOP5·결승타·상대전적 |
...
- `DOOSAN_SECTION_THEME` 빌드 플래그로 두산 섹션 아이콘 세트를 고정
- external Cheolwoong 버전은 두산 철웅이 섹션 아이콘 사용
- external Mangom 버전은 두산 망곰 섹션 아이콘 사용
